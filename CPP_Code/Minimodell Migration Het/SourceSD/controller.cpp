...
void Controller::init(int seed, int columns, int rows, double migration_proportion,double death_proportion, double custom_feedingrange_min, double custom_feedingrange_var, int scenario, int maximum_species, bool fdo)
{
  ...
  //Resource und ersten Spezies erstellen:
  S = patches+1;
  species_count[0] = patches;
  global_species[0] = new Species(0.0, 0.0, 0.0);
  global_species[0]->set_index(0);
  
  //Erstellen einer zufälligen Spezies wobei um Körpermasse 2.0 "mutiert" wird für jeden Patch
  for (int i; i<patches; i++)
  {
    species_count[i+1] = 1;
    
    double start_species_bm;
    double start_species_fc;
    double start_species_fr; 
    
    bool create=true;
    
    //Suche so lange Spezies Parameter bis sie überlebensfähig ist
    while(create)
    {
      
      start_species_bm = 2.0 + 2.0*log10(5.0)*(gsl_rng_uniform(r)-0.5);
      start_species_fc = start_species_bm - 1.0 - 2.0*gsl_rng_uniform(r);
      start_species_fr = feedingrange_min+feedingrange_var*gsl_rng_uniform(r);    
    
      if ( abs(start_species_fc) < start_species_fr )
      {
	create = false;
      }
    ...
    global_species[i+1] = new Species(start_species_bm, start_species_fc, start_species_fr);
    global_species[i+1]->set_index(i+1);
  }
  
  species_pos = patches+2;
  ...
  for(int i = 0; i < patches; i++)
  {
    ...
    webs[i]->add_species(global_species[i+1], 0);
