...
def filter_list_for_timestamp(time_stamp, f_read, line_list):

  while True:
  
    templine = f_read.readline()
    data_raw = templine.strip('\n').split("\t")
    ...
    for j in range(3, len(data), 3):				#Für jeden Eintrag prüfe ob zu bestimmtem Zeitpunkt spezies gelebt hat, wenn nicht, lösche Species aus Liste
      if data[j+1] <= time_stamp and data[j+2] >= time_stamp and data[0] != 0.:
