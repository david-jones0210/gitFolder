...
{
  const int param_count = 8;		//Anzahl der Auszuwertenden Größen, ergibt sich aus den Festlegungen in collect_params()
...
void Output::collect_params(void* foodweb)
{
  ...
  *ev_params[6] << web->get_connectance();	// Connectance des Netzwerks, = #links / dim^2
  *ev_params[7] << web->get_feedingrange();	// Mittlere feeding range aller Spezies im Netz
}


void Output::open_files(string path)
{
  string c[] = {"par", "bm", "c", "s", "tropos", "alpha", "trophdistri"};
...
void Output::print_line(resfile_type f, int dim, int time, int patch, int array_size, void* data)
{
