...
double Foodweb::get_feedingrange()
{
  Eval ev;
  for(int i = 1; i < dim; i++) //i=1 um ressource nicht mit einzubeziehen
  {
    ev << species[i]->feedingrange;
  }
  return ev.get_mean();
}
...
void Foodweb::calc_alpha(double x)
{
  for(int i = 0; i < dim; i++)
  {
	...
	alpha[i][j] = 0.0;
      ...
      {
	if (species[i]-> feedingrange>0 && j == 0)
	  alpha[i][j] = pow(x, trophical_level[i]) * (1.0 - competition[j])*3;
	else if (species[i]-> feedingrange>0)
	  alpha[i][j] = pow(x, trophical_level[i]) * (1.0 - competition[j]) / (5 * species[i]->feedingrange);
	else
	  cout << "feedingrange <= 0! species index: " << i << endl;
      }
    }
  }
  return;
